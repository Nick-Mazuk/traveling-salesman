...
export class Algorithms {
    ...
    static annealing(
        ...
        let maxTime =
            tour.cities.length * 100 + (tour.cities.length > 10 ? (tour.cities.length - 10) ** largeMultiplier : 0)
        let frame = 0
    ...
    static force(tour: Tour, cityGrid): Tour {
        if (tour.cities.length > 9) {
            alert(
                'Brute force is slow. Very slow. It will likely take minutes if not much longer, so the algorithm was aborted. Please delete cities or choose another algorithm.'
            )
            return tour
        }
        let shortestLength = tour.length(cityGrid)
