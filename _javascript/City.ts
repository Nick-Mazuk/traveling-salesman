...
export class City {
    ...
    draw(ctx: CanvasRenderingContext2D, cityGrid: boolean, blockSize: number): void {
        if (cityGrid) {
            const imageYSize = blockSize * 0.5
    ...
    distanceFromCity(city: City, cityGrid: boolean): number {
        if (cityGrid) {
            ...
            if (isInSameBlockColumn && !shareSameRoad) {
                if (this.blockColumnPosition == 1 && city.blockColumnPosition == 1) {
                    // middle and outer
    ...
    move(
        ...
        if (xPos > this.canvas.getBoundingClientRect().width - City.radius)
            xPos = this.canvas.getBoundingClientRect().width - City.radius
        if (yPos > this.canvas.getBoundingClientRect().height - City.radius)
            yPos = this.canvas.getBoundingClientRect().height - City.radius
        this.xPos = xPos
        this.yPos = yPos
