...
let algorithmDescriptions = {
    ...
    greedy:
        '<b>Greedy Algorithm</b>: Each node goes to the next closest node, finds suboptimal path excluding trivial cases',
...
function canvasClicked(e: MouseEvent): void {
    ...
    if (!rightClicked) {
        ...
        if (!clickedCities.length) {
            ...
            if (cityGrid) tour.draw(ctx, blocks, blockXSize, blockYSize, cityGrid)
        }
    }
            ...
            if (currentMousePosition[0] != selectedCity.xPos || currentMousePosition[1] != selectedCity.yPos) {
                if (City.isPointInBlock(currentMousePosition, ctx, dpr)) {
                    if (tour.positionInCity(currentMousePosition[0], currentMousePosition[1]).length == 0) {
                        ...
                        const isAnchorAbove = City.isPointInBlock(
                            [currentMousePosition[0], currentMousePosition[1] - blockYSize],
                            ctx,
                            dpr
                        )
                        if (isAnchorAbove) {
...
function randomizeCities(cityCount: number) {
    ...
    if (!cityGrid) {
        for (let i = 0; i < cityCount; i++) {
            const city = new City(
                Math.floor(Math.random() * (width - 2 * cityRadius)) + cityRadius,
                Math.floor(Math.random() * (height - 2 * cityRadius)) + cityRadius,
                canvas
            )
            tour.addCity(city)
        }
        ...
        while (tempCities.length < revisedCityCount) {
            ...
            if (tempCities.filter(city => city[0] == xPos && city[1] == yPos).length == 0) tempCities.push([xPos, yPos])
        }
...
window.addEventListener('load', () => tour.draw(ctx, blocks, blockXSize, blockYSize, cityGrid))
